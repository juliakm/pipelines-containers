...
read -p "Enter Agent pool (Press enter for Default) " AZP_AGENT_POOL
echo ${AZP_AGENT_POOL:=Default}
...
az container create --resource-group ${RESOURCE_GROUP} --name ${ACI_NAME} --image ghcr.io/juliakm/pipelines-containers:main --dns-name-label aci-acr-demo --ports 80 --environment-variables 'AZP_TOKEN'='${AZP_TOKEN}' 'AZP_AGENT_NAME'='${AZP_AGENT_NAME}' 'AZP_POOL'='${AZP_POOL}' 'AZP_URL'='https://dev.azure.com/${AZP_ORG}'